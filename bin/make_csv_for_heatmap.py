...
def save_to_csv(samples, sample_counts, sample_clade_counts, totals, out_csv, min_read_count=10):
    with open(out_csv, 'w') as out:
        out.write("sample,taxon,taxon_ncbi,taxon_rank,simple_taxon_rank,num_reads,num_clade_reads,sample_total,score,case_frequency,control_frequency,case_max_read_count,control_max_read_count,clade_score,clade_case_frequency,clade_control_frequency,clade_case_max_read_count,clade_control_max_read_count\n")
        for taxon in sample_counts:
...
def main():
    ...
    parser.add_argument("--output", help="CSV file summarizing counts", required=True)
    parser.add_argument("--report_template", help="HTML template for report", required=True)
    ...
    for group in groups:
        ...
        save_to_csv(list(groups["all"].keys()), sample_counts, sample_clade_counts, totals, args.output, args.min_num_reads)
